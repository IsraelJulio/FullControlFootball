...
builder.Property(x => x.NameSnapshot).HasMaxLength(180).IsRequired();
...
builder.HasOne(x => x.CareerSave).WithMany(x => x.SeasonCompetitions).HasForeignKey(x => x.CareerSaveId).OnDelete(DeleteBehavior.Cascade);
builder.HasOne(x => x.Season).WithMany(x => x.SeasonCompetitions).HasForeignKey(x => x.SeasonId).OnDelete(DeleteBehavior.Cascade);
builder.HasOne(x => x.Competition).WithMany().HasForeignKey(x => x.CompetitionId).OnDelete(DeleteBehavior.Restrict);
