...
builder.Property(x => x.PrimaryPositionSnapshot).HasMaxLength(20);
...
builder.HasOne(x => x.CareerSave).WithMany(x => x.SavePlayers).HasForeignKey(x => x.CareerSaveId).OnDelete(DeleteBehavior.Cascade);
builder.HasOne(x => x.Player).WithMany().HasForeignKey(x => x.PlayerId).OnDelete(DeleteBehavior.Restrict);
builder.HasOne(x => x.CurrentClub).WithMany().HasForeignKey(x => x.CurrentClubId).OnDelete(DeleteBehavior.SetNull);
