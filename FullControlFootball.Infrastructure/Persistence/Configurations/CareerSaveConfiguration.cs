...
builder.Property(x => x.GameEdition).HasMaxLength(50).IsRequired();
builder.Property(x => x.Description).HasMaxLength(2000);
builder.HasIndex(x => x.UserId);
builder.HasIndex(x => new { x.UserId, x.Name });
builder.HasOne(x => x.User).WithMany(x => x.CareerSaves).HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Cascade);
builder.HasOne(x => x.MainClub).WithMany().HasForeignKey(x => x.MainClubId).OnDelete(DeleteBehavior.SetNull);
