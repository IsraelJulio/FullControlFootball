...
builder.Property(x => x.ClubNameSnapshot).HasMaxLength(180).IsRequired();
...
builder.HasOne(x => x.CompetitionStanding).WithMany(x => x.Rows).HasForeignKey(x => x.CompetitionStandingId).OnDelete(DeleteBehavior.Cascade);
