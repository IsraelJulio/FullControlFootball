...
public sealed class CompetitionTopAssistConfiguration : IEntityTypeConfiguration<CompetitionTopAssist>
{
    public void Configure(EntityTypeBuilder<CompetitionTopAssist> builder)
    {
        builder.ToTable("competition_top_assists");
        builder.HasKey(x => x.Id);

builder.Property(x => x.PlayerNameSnapshot).HasMaxLength(180).IsRequired();
builder.Property(x => x.ClubNameSnapshot).HasMaxLength(180);
builder.HasIndex(x => x.SeasonCompetitionId);
builder.HasIndex(x => new { x.SeasonCompetitionId, x.SnapshotDateUtc, x.Position });
builder.HasOne(x => x.SeasonCompetition).WithMany(x => x.TopAssists).HasForeignKey(x => x.SeasonCompetitionId).OnDelete(DeleteBehavior.Cascade);
