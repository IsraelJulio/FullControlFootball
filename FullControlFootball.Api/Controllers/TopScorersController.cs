...
public sealed class TopScorersController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCompetitionTopScorersRequest request, CancellationToken cancellationToken)
    {
        var items = await _rankingCommandService.CreateTopScorersAsync(_currentUserAccessor.GetRequiredUserId(), request, cancellationToken);
        return Ok(items);
    }
}
